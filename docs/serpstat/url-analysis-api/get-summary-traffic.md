...
The `SerpstatUrlProcedure.getSummaryTraffic` method returns traffic and keyword statistics for website pages that match a specific URL mask.
...
**POST** `https://api.serpstat.com/v4/#SerpstatUrlProcedure.getSummaryTraffic`
...
| **method** | string | ✅ | Should be exact `SerpstatUrlProcedure.getSummaryTraffic` |
| **params** | object | ✅ |  |
...
| **domain** | string | ✅ | The domain for which to retrieve traffic and keyword data |
| **urlContains** | string | ✅ | Searched part of URL |
| **output_data** | string | ❌ | Output data value. Both options enable by default, choose one to spend less credits. |
...
| **id** | string | ✅ | Request / response identifier, prodvided from your side, will be mirrored from request to `id` in response. Example `"id":"123"` |
| **result** | object | ✅ |  |
| **urls** | integer | ✅ | The number of URLs matching the given mask |
| **traffic** | integer | ❌ | Organic traffic found by URL mask |
| **keywords** | integer | ❌ | Number of keywords found by URL mask |
| **summary_info** | object | ❌ |  |
...
```bash
curl --request POST \
  --url 'https://api.serpstat.com/v4/?token=123#SerpstatUrlProcedure.getSummaryTraffic' \
...
- **Method**: [`SerpstatUrlProcedure.getSummaryTraffic`](https://api.serpstat.com/v4/#SerpstatUrlProcedure.getSummaryTraffic)
- **Credit Cost**: 1000 credits per parameter in `output_data`
- **Purpose**: Get traffic and keyword statistics for URL patterns
- **Use Case**: Analyze specific sections of websites (e.g., mobile pages, product categories)
- **Parameters**: Required - `se`, `domain`, `urlContains`; Optional - `output_data` (traffic/keywords)