...
The `SerpstatDomainProcedure.getAdsCompetitors` method brings in the list of domain competitors in PPC.
...
You can get no more than 1000 results per one request.
...
| `method` | string | Yes | Should be exact `SerpstatKeywordProcedure.getAdsCompetitors` |
...
| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `params.sort` | object | No | Order of sorting the results in the format: `"field: order"` |
| `params.page` | integer | No | Page number in response |
...
| `result.data` | array[object] | main results |
| `result.summary_info` | object | Pagination information |
...
| `ads` | integer | Number of ads for this domain |
| `all` | integer | Total number of results for this domain |
...
```bash
curl --request POST \
  --url 'https://api.serpstat.com/v4/?token=123#SerpstatKeywordProcedure.getAdsCompetitors' \
...
The response contains an array of competitor domains with their advertising metrics:

- **domain**: The competitor domain name
- **ads**: Number of ads found for this domain
- **all**: Total number of results (ads + organic) for this domain
- **common**: Number of keywords common between your query and this domain
