# addKeywordList | SERP Crawling API

## Overview

The [`tasks.addKeywordList`](https://serpstat.com/rt/api/v2/#tasks.addKeywordList) method allows sending keywords with commas to crawl regular SERP and local results as an array.
...
- **URL**: `https://serpstat.com/rt/api/v2/#tasks.addKeywordList`
- **Method**: `POST`
...
| `method` | string | Yes | Should be exact `tasks.addKeywordList` |
...
| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `keywords` | array[string] | Yes | Keywords with commas for parsing. |
...
```bash
curl --request POST \
  --url 'https://serpstat.com/rt/api/v2/?token=123#tasks.addKeywordList' \
...
- This method accepts an array of keyword strings with commas, unlike `addTask` which accepts a single comma-separated string
- Each keyword string in the array consumes credits based on the search engine, country, and other parameters
