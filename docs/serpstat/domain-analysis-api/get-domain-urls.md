# getDomainUrls | Serpstat Public API

The `SerpstatDomainProcedure.getDomainUrls` method returns the list of URLs within the analyzed domain and the number of keywords for each URL.
...
| **method** | string | required | Should be exact `SerpstatDomainProcedure.getDomainUrls` |
| **params** | object | required |  |
| **params.domain** | string | required | Domain name |
...
| **params.sort** | object | optional | Order of sorting the results in the format: {field: order} |
...
| **result.data.0.url** | string | required |  |
| **result.data.0.keywords** | integer | required |  |
...
```bash
curl --request POST \
  --url 'https://api.serpstat.com/v4/?token=123#SerpstatDomainProcedure.getDomainUrls' \
